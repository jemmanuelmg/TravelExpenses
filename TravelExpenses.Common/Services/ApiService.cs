...
namespace TravelExpenses.Common.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<Response> AddNewTravel<T>(string urlBase, string servicePrefix, string controller, string tokenType, string accessToken, T travelRequest)
        {
            try
            {
                string request = JsonConvert.SerializeObject(travelRequest);
                StringContent content = new StringContent(request, Encoding.UTF8, "application/json");
                HttpClient client = new HttpClient
                {
                    BaseAddress = new Uri(urlBase)
        ...
        public async Task<Response> AddNewExpense<T>(string urlBase, string servicePrefix, string controller, string tokenType, string accessToken, T expenseRequest)
        {
            try
            {
